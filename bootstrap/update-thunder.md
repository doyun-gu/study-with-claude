# /update-thunder — Update study-with-claude to latest version

Run the update script:

```bash
bash "$(dirname "$(readlink "$HOME/.claude/commands/where-is-god.md")")/update.sh"
```
...
If the output contains `__MERGE_SETTINGS__`, the target already has `.claude/settings.local.json`. Merge the `permissions.allow` arrays:
...
Everything was handled by the script. No further action needed — just confirm the summary.
